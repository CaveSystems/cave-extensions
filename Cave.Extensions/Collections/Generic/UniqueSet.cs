...
/// <summary>
/// Gets a typed 2D set. This class uses a Set for each dimension allowing only unique values in each dimension. Additionally to the fast A (Key1) and B (Key2)
/// lookup it provides indexing like a list.
/// </summary>
    ...
    where TKey2 : notnull
{
    ...
    public int IndexOfA(TKey1 key) => !lookupA.TryGetValue(key, out var item) ? -1 : list.IndexOf(item);
    ...
    /// <returns>The index of item if found in the list; otherwise, -1.</returns>
    public int IndexOfB(TKey2 value) => !lookupB.TryGetValue(value, out var item) ? -1 : list.IndexOf(item);
