...
namespace Cave
{
    ...
    public class SemVer : IEquatable<SemVer>, IComparable<SemVer>, IComparable
    {
        ...
        public static bool TryParse(string text, bool throwEx, out SemVer version)
        {
            ...
            if (!result)
            {
                if (throwEx) throw new InvalidOperationException($"Invalid SemanticVersion.Core '{value}'!");
            }
#endif
            ...
            {
                result = false;
                if (throwEx) throw new InvalidOperationException($"Invalid SemanticVersion.Build '{build}'!");
            }
            if (preRelease?.HasInvalidChars(ValidCharsMetaParts) is true)
            {
                result = false;
                if (throwEx) throw new InvalidOperationException($"Invalid SemanticVersion.PreRelease '{preRelease}'!");
            }
            return result;
        }
        ...
        public Version Core => Patch < 0 ? new Version(Major, Minor) : new Version(Major, Minor, Patch);
        ...
        /// <summary>
        /// Returns the version without the <see cref="Build"/> part.
        /// </summary>
