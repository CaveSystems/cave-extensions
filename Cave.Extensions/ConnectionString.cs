...
/// <summary>Gets a connection string parser. The following string will be parsed: [protocol://][user[:password]@]server[:port][/path/to/somewhere].</summary>
public struct ConnectionString : IEquatable<ConnectionString>
{
    ...
    /// <summary>Parses a specified connection string of the form [protocol://][user[:password]@]server[:port][/path/to/somewhere[?option=value[ &amp;option=value]]].</summary>
    /// <param name="connectionString">The connection string.</param>
        ...
        string defaultServer, ushort? defaultPort, string defaultPath, string defaultOptions)
    {
        ...
        // get protocol
        var parts = connectionString.Trim().Split(["://"], 2, StringSplitOptions.None);
        if (parts.Length > 1)
        {
            protocol = parts[0];
        }
        ...
        if (parts.Length > 1)
        {
            ...
            parts = usernamePassword.Split([':'], 2);
            username = parts[0];
            if (parts.Length > 1)
            {
                password = parts[1];
    ...
    /// <summary>Gets the connection string with credentials (username and password). If you want to strip some parts of the connection string use <see cref="ToString(ConnectionStringPart)"/>.</summary>
    /// <returns>Returns a new string.</returns>
    public override string ToString() => ToString(ConnectionStringPart.All);

    /// <summary>
    /// Gets an <see cref="Uri"/> for the <see cref="ConnectionString"/>. Only <see cref="ConnectionString"/> s with the Protocol file, ftp, http, https,
    /// mailto, news and nntp may be converted to an <see cref="Uri"/>!.
    /// </summary>
    /// <param name="items">The connection string parts.</param>
