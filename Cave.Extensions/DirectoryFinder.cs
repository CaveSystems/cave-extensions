...
public class DirectoryFinder
{
    ...
    {
        if (BaseDirectory is null) throw new InvalidOperationException("BaseDirectory is null!");
        try
        {
            if (++depth > maxDepth)
            {
            ...
            try
            {
                dirs = directoryMask is null ? Directory.GetDirectories(current.FullPath) : Directory.GetDirectories(current.FullPath, directoryMask);
            }
            catch (Exception ex)
            {
